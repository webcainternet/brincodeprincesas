...
class ModelMercadolivreMercadolivre extends Model
{
	...
	public function synchronizeQueue($product_id) {

	   $product_info=$this->db->query("SELECT * FROM " . DB_PREFIX . "mercadolivre_sync_queue WHERE product_id = '" . (int) $product_id. "'")->row;

	   if(!$product_info) {
	      $this->db->query("INSERT INTO " . DB_PREFIX . "mercadolivre_sync_queue SET product_id = '" . (int)$product_id . "'");
	   }
	...
	public function getProductFromSyncQueue() {
	   ...
	   $rows=$this->db->query("SELECT * FROM " . DB_PREFIX . "mercadolivre_sync_queue limit 0,5")->rows;
	...
	public function synchronizeProduct($product_id, $updateOnly=false)
	{
	   ...
	   if(empty($product['mercaId'])) {

	   	 if($this->config->get('mercadolivre_detect_cat') && $product['name']) {

	   	    $predictCat = $this->mercadolivre->getMLCategoryPredict($product['name']);

	   	    if($predictCat && $predictCat['id']) {
 				$product['mercaId']= $predictCat['id'];
	   		    $cat_data['mercaId'] = $predictCat['id'];
	   		    $cat_data['mercaTree'] = $predictCat['mercaTree'];
	   ...
	   if(empty($product['mercaId'])) {
	   	    	$product['mercaId']=$this->config->get('mercadolivre_mercaId');
	   		    $cat_data['mercaId'] = $this->config->get('mercadolivre_mercaId');
	   		    $cat_data['mercaTree'] = $this->config->get('mercadolivre_category_tree');
	   		    $cat_data['product_id'] = $product_id;
	   }
				...
				elseif($updateOnly===false)
				{
				  ...
				  if($product['mercaVideoId']){
				   ...
				   if(!empty($mercaId))
				   {
					  ...
					  if($cat_data) {
					  	 $this->mercadolivre->setMLCategory($cat_data);
					  }
	...
	public function install(){
        ...
        $sql = "
            CREATE TABLE IF NOT EXISTS `".DB_PREFIX."mercadolivre_sync_queue` (
              `id` bigint(11) NOT NULL AUTO_INCREMENT,
    ...
    public function uninstall(){
         ...
         $query = $this->db->query("DROP TABLE IF EXISTS `".DB_PREFIX."mercadolivre_sync_queue`");
         $this->log->write('MERCADOLIVRE Module --> Dropped '.DB_PREFIX.'mercadolivre_sync_queue table');
	...
	public function isDBBUPdateAvail(){
	      ...
	      $tables=array('mercadolivre_product','mercadolivre_category','mercadolivre_status','mercadolivre_listing_type','mercadolivre_order','mercadolivre_api_call','mercadolivre_attr_combination','mercadolivre_attribute','mercadolivre_sync_queue');
